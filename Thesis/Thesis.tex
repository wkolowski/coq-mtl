...
The older approach, which is more ad hoc, amounts to implementing monads and various related structures at the library level and using them to track effects using the type system.
...
This deficiency can be remedied by introducing a more abstract design based on typeclasses. Here each effect \m{E} is split into a specification and an implementation. The specification is a typeclass expressing that a monad \m{M} supports an effect \m{E} and the various monad transformers can then implement this typeclass.
...
Another problem, far bigger and more conceptual, is the order of effects: if a monad is both stateful and nondeterministic, how do these two effects interact? Is there a global state or does each branch of the computation have its own local state?

The problems with effect ordering lead to the idea that maybe there shouldn't be an ordering, or rather that effects should be explicitly handled to determine the ordering or even discard or reinterpret some of them. But this requires the effects to become just pieces of syntax, a language that expresses a signature of what an effect can do. This leads us down the rabbit hole to the idea of free monads and recent development like Haskell's \m{extensible-effects} library.
...
. It boils down to having a \textbf{type and effect system}, in which every expressions is associated with a type and a list or set of effects that it may cause. Values are then those expressions where this list/set is empty and computations are all the rest.
