...
\chapter{Implementation, theorems and examples}
...
We finish this chapter with two examples of reasoning with our monads and effects. First we revisit the example from chapter 1 and see how it can be improved with do-notation and automation tactics. We finish with a formalization of the fast product example from \cite{JustDoIt}.
...
\begin{listing}[H]
    ...
    \caption{The tree labeling example from chapter 1, generalized with the \m{MonadState} typeclass and improved with do-notation}
    \label{Example1}
\end{listing}
...
This may look unimpressive, because \m{work} is implemented using a simple case split, without recursion. We could also implement \m{work} recursively, by checking if the head of the list is zero and failing in this case, otherwise muliplying the product of the tail by the head.
