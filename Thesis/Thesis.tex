...
There is one implementation of effects in Coq, available at \cite{CoqIO}, that allows one to define programs with IO and even concurrency and then extract the code to OCaml. This development is, however, rather obscure (there is not even a single paper about it). There are also an implementation of exceptions by means of a translation from the Calculus of Constructions to itself, realized in Coq as a plugin \cite{FailureIsNotAnOption}.

Besides, there has been some work on formally verifying programs with algebraic effects and handlers in Coq. \cite{CoqEff1} \cite{CoqEff2}

Among dependently typed languages most similar to Coq, only Idris seems to have a consciously designed effect system \cite{IdrisEffects}. Another, less similar language, that treats dependent types and effects natively is F* \cite{FStar}.
