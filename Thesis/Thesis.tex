...
Canonical structures are a mechanism unique to Coq. They were introduced in a PhD thesis written in French \cite{CanonicalStructures0} and remain quite obscure since then, but see \cite{CanonicalStructures1} \cite{CanonicalStructures2} \cite{CanonicalStructures3}. They are built on the foundation of parametrized dependent records, just like typeclasses, but they differ from the latter in the instance search mechanism -- canonical structures are found using unification and not ad hoc proof search.
...
It is precisely these inductive type definitions that forbid Coq modules from being first-class (it may come as a surprise, but defining a new type can be considered an effect!). Modules are also often used as namespaces, because all names declared in a module must be accessed by prefixing them with the module name, unless the module is imported into scope.


Most of the basic design choices (the class hierarchy up to monad transformers, naming of modules nad functions, etc.) were made to match Haskell, whereas the monadic effect classes and their laws are taken from \cite{JustDoIt}. This results in the hierarchy of classes and their instances shown in figure \ref{fig:hierarchy}.
