...
3. A description of the inner workings of the library.
3.1. File structure.
3.2. Design choices:
3.2.1. Practical comparison with 
Experimenting with Monadic Equational Reasoning in Coq.
3.2.2. Applicative-monad proposal etc.
3.3. Implementation.
...
5. More: a case study in proof engineering - how do the tactics hs, monad.
...
TODO: wrzucić to do OPAMa

\section{Further work}

There's still some work to be done. The exact things to do can be found by looking at the \m{TODO} section in \m{README.md} and by searching the source code for the keywords \m{Abort}, \m{Admitted} and for the marker TODO.
...
Notations could get some love too. The current do notations has the ugly 
\m{;;} for computations that don't bind their result and I couldn't even define the idiom brackets of Idris using Coq's notation mechanism.
...
Another possibility is to rewrite the whole automation engine, so as to use proof by reflection \cite{ProofByReflection}, which is more reliable than just using Ltac. The beginning of this has been already accomplished with the tactic \m{reflect\_functor}, which implements basic principles of reasoning with functors.

Last but not least, as already hinted at, \libname\ is a great basis on which a library similar to Haskell's \m{extensible-effects} may be implemented.
