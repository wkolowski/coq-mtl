...
| Comp : forall A B C : type, Exp (TArr A B) -> Exp (TArr B C) -> Exp (TArr A C)
...
| fcons : forall A B C : type, Exp (TArr A B) -> flist B C -> flist A C.
...
Lemma flistDenote_efmap :
  forall (A B : type) (l : flist A B),
    flistDenote (efmap (@Fmap) l) = fmap (flistDenote l).
Proof.
  intros.
  funelim (efmap (@Fmap) l); simp flistDenote.
  - now rewrite fmap_id.
  - simp efmap flistDenote.
...
Proof.
  intros.
  funelim (flatten e); simp flistDenote; only 1-2, 4: easy.
  - now rewrite flistDenote_fapp, H, H0.
  - now rewrite flistDenote_efmap, H.
Qed.
...
Lemma reflect_functor :
  forall (A B : type) (e1 e2 : Exp (TArr A B)),
    flistDenote (flatten e1) = flistDenote (flatten e2) ->
      denote e1 = denote e2.
Proof.
  now intros; rewrite <- flistDenote_flatten, H, flistDenote_flatten.
Qed.
...
Ltac reifyType T :=
match T with
| ?F ?A => let t := reifyType A in constr:(TF t)
| ?A    => constr:(TVar A)
end.
...
| |- ?e1 = ?e2 =>
  ...
  let e2' := reify e2 in
    ...
    repeat (simp flatten || simp efmap || simp fapp || simp flistDenote);
