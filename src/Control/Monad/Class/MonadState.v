...
Proof.
  intros.
  now rewrite <- bind_assoc, put_put'.
Qed.
...
Proof.
  intros.
  now rewrite <- constrA_spec, put_get, constrA_spec.
Qed.
...
Definition state {A : Type} (f : S -> (A * S)%type) : M A := do
  s <- get;
  let '(a, s') := f s in
  do
    put s';;
...
Definition modify (f : S -> S) : M unit := do
  s <- get;
  put $ f s.

Definition gets {A : Type} (f : S -> A) : M A := do
  s <- get;
  pure $ f s.
...
Proof.
  ...
  rewrite <- get_put, bind_assoc.
  f_equal.
  ext s'.
  rewrite <- !bind_assoc, (put_get' _ _ _ _ (f s')).
  now hs.
Qed.
