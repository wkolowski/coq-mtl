...
*)
Definition fmap_Cont {R A B : Type} (f : A -> B) (m : Cont R A) : Cont R B :=
  fun g : B -> R => m (f .> g).
...
(**
  ...
  continuation.
*)
Definition pure_Cont {R A : Type} (a : A) : Cont R A :=
  fun f : A -> R => f a.
...
Proof.
  intros H.
  destruct (H bool).
  specialize (ap_comm bool bool _ (fun _ => id) (const true) (const false)); compute in ap_comm.
  apply (f_equal (fun f => f id)) in ap_comm.
...
Definition callCC'
  {R A : Type} (f : (forall B : Type, A -> Cont R B) -> Cont R A) : Cont R A :=
