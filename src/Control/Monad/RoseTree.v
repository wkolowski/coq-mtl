...
Proof.
  all: now intros; ext t;
    induction t as [x | l IHl r IHr]; cbn; rewrite ?IHl, ?IHr.
Defined.
...
Proof.
  all: cbn; intros.
  - now rewrite (@fmap_id _ Functor_RT).
  - induction g as [g | gl IHgl gr IHgr]; cbn.
    + induction f as [f | fl IHfl fr IHfr]; cbn.
      * now rewrite (@fmap_comp' _ Functor_RT).
      * now rewrite IHfl, IHfr.
    + now rewrite IHgl, IHgr.
  - easy.
  - now induction f as [f | l IHl r IHr]; cbn; rewrite ?IHl, ?IHr.
  - easy.
Defined.
...
Proof.
  ...
  specialize (ap_comm bool bool _ (fun _ => id)
    (Node (Leaf false) (Leaf true)) (Node (Leaf true) (Leaf false))); compute in *.
  now congruence.
Qed.
...
Proof.
  all: cbn; intros.
  - easy.
  - now induction ma as [a | l IHl r IHr]; cbn; rewrite ?IHl, ?IHr.
  - now induction ma as [a | l IHl r IHr]; cbn; rewrite ?IHl, ?IHr.
  - induction mf as [f | fl IHfl fr IHfr]; cbn.
    + induction mx as [x | xl IHxl xr IHxr]; cbn; [easy |].
      now rewrite IHxl, IHxr.
    + now rewrite IHfl, IHfr.
Defined.
...
Fixpoint foldMap_RT {A : Type} {M : Monoid} (f : A -> M) (t : RT A) : M :=
match t with
| Leaf x   => f x
...
Proof.
  intros.
  ext t.
  induction t as [| l IHl r IHr]; unfold compose in *; cbn; [easy |].
  now rewrite IHl, IHr.
Defined.
