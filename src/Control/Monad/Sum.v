...
*)
Definition fmap_Sum {E A B : Type} (f : A -> B) (x : sum E A) : sum E B :=
match x with
| inl a => inl a
| inr b => inr (f b)
end.
...
(**
  ...
  successful.
*)
Definition pure_Sum {E A : Type} (x : A) : sum E A :=
  inr x.
...
*)
Definition ap_Sum {E A B : Type} (sf : sum E (A -> B)) (sa : sum E A) : sum E B :=
match sf, sa with
| inl a, _     => inl a
| _    , inl a => inl a
| inr f, inr x => inr (f x)
...
Proof.
  intros H.
  destruct (H bool).
  specialize (ap_comm nat nat nat (fun _ => id) (inl true) (inl false)); compute in ap_comm.
  congruence.
Qed.
...
*)
Definition bind_Sum {E A B : Type} (sa : sum E A) (f : A -> sum E B) : sum E B :=
match sa with
| inl e => inl e
| inr a => f a
end.
...
#[refine]
#[export]
Instance MonadFail_Sum (E : Type) (e : E) : MonadFail (Sum E) (Monad_Sum E) :=
{
  fail := @fail_Sum E e;
}.
Proof.
...
#[refine]
#[export]
Instance MonadExcept_Sum (E : Type) (e : E) : MonadExcept (sum E) (Monad_Sum E) :=
{
...
Proof.
  - now cbn.
  - intros A []; compute; [| easy].
    admit.
  - now intros A [] []; cbn.
  - now intros A x []; compute.
Abort.

Definition foldMap_Sum {E A : Type} {M : Monoid} (f : A -> M) (x : sum E A) : M :=
match x with
| inl _ => neutr
| inr a => f a
end.
