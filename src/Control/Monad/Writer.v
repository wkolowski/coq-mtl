...
*)
Definition Writer (W : Monoid) (A : Type) : Type :=
  (A * W)%type.
...
*)
Definition pure_Writer {W : Monoid} {A : Type} (a : A) : Writer W A :=
  (a, neutr).
...
Definition ap_Writer
  {W : Monoid} {A B : Type} (wf : Writer W (A -> B)) (wa : Writer W A) : Writer W B :=
    let (f, w) := wf in
    let (a, w') := wa in
      (f a, op w w').
...
Proof.
  intros H.
  destruct (H (Monoid_list_app bool)).
  unfold Writer in ap_comm.
...
Definition bind_Writer
  {W : Monoid} {A B : Type} (wa : Writer W A) (f : A -> Writer W B) : Writer W B :=
    let (a, w) := wa in
    let (b, w') := f a in
      (b, op w w').
...
#[refine]
#[export]
Instance MonadWriter_Writer
  (W : Monoid) : MonadWriter W (Writer W) (Monad_Writer W) :=
{
  tell := fun w => (tt, w);
